...
class VidStab:
    ...
    def _init_is_complete(self, gen_all):
        ...
        max_ind = min([self.frame_queue.max_frames, self.frame_queue.max_len])
    ...
    def _init_writer(self, output_path, frame_shape, output_fourcc, fps):
        h, w = frame_shape
        self.writer = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*output_fourcc), fps, (w, h), True)
    ...
    def _apply_transforms(self, output_path, max_frames, use_stored_transforms,
        ...
        while True:
            ...
            combined_frame = np.hstack((frame_i.image, transformed))

            break_playback = general_utils.playback_video(combined_frame, True, delay=min([self._smoothing_window, max_frames]))
            if break_playback:
                break
            ...
            if output_path:
                if self.writer is None:
                    self._init_writer(output_path, transformed.shape[:2], output_fourcc, fps=fps_force)
